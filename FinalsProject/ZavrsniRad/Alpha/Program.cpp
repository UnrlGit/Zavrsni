...
// Vertex Shader
static const char* vShader = "Shaders/basic.vert";
//Fragment shader
static const char * fShader = "Shaders/basic.frag";
...
void Render(GLuint &uniformModel, GLuint &uniformProjection, GLuint &uniformView, GLuint &uniformEyePosition, GLuint &uniformSpecularIntensity, GLuint &uniformShininess, glm::mat4 projectionMatrix, glm::mat4 viewMatrix)
{
	...
	uniformShininess = shaderList[0]->GetSpecularPowerLocation();
...
int main(int argc, char** argv)
{
	...
	SoundEngine->play2D("Audio/breakout.wav");
	while (mainWindow.GetWindowShouldClose() == false)
	{
		
		// fps test
		showFPS(mainWindow.GetWindow());
...
void showFPS(GLFWwindow* window)
{
	...
	secondsPassed = currentSeconds - previousSeconds;

	if (secondsPassed > 0.25)
	{
		previousSeconds = currentSeconds;
		double fps = (double)frameCounter / secondsPassed;
		double msPerFrame = 1000.0 / fps;

		std::ostringstream outs;
		outs.precision(3);
		outs << std::fixed << "ALPHA NAME_REPLACE" << "    " << "FPS: " << fps
